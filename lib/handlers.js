                        ...
                        _data.delete('users', phone, (err) => {
                            if(!err){
                                ...
                                if(userChecks.length > 0){
                                    ...
                                    userChecks.map(id => _data.delete('checks', id, (err) => {
                                        if(!err){
                                            deleteCount++;
                                        } else {
                                            deleteError = true;
                                    ...
                                    if(!deleteError){
                                        ...
                                        callback(500, {'Error' : `Not all checks have been deleted, only ${deleteCount} deleted`});
                                    }
                                } else {
                                    callback(200);
                                }
                    ...
                    if(tokenIsValid) {
                        ...
                        _data.update('checks', id, checkData, (err) => {
                            if(!err){
                                ...
                                callback(500, {'Error' : 'Could not put update check'});
                            }
                        });
                    } else {
                        callback(403, {'Error' : 'Missing required token in header, or token is expired'});
