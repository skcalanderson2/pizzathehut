...
// Optional Data: none
handlers._checks.post = (err, callback) => {
    const protocol = typeof(data.payload.protocol) == 'string' && data.payload.protocol.trim().length == 10 ? data.payload.protocol.trim() : false; 
    const url =  typeof(data.payload.url) == 'string' && data.payload.url.trim().length > 0 ? data.payload.url.trim() : false;
    const method =  typeof(data.payload.method) == 'string' && data.payload.method.trim().length > 0 ? data.payload.method.trim() : false;
    const successCodes =  typeof(data.payload.successCodes) == 'string' && data.payload.successCodes.trim().length > 0 ? data.payload.successCodes.trim() : false;
    const timeoutSeconds =  typeof(data.payload.timeoutSeconds) == 'number' && data.payload.timeoutSeconds > 0 ? data.payload.timeoutSeconds : false;
