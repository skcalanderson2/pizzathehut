            ...
            checks.map(check => {
                _data.read('checks', check, (err, originalCheckData) => {
                    if(!err && originalCheckData){
                        ...
                        debug(`Error reading ${check}'s data` );
                    }
                });
            })
        } else {
...
workers.validateCheckData = (originalCheckData) => {
    ...
    if(originalCheckData.id 
        ...
        && originalCheckData.timeoutSeconds){
            ...
            debug("Error: One of the checks it not properly formatted");
        }
}
...
workers.alertUserToStatusChange = (newCheckData) => {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
        if(!err){
            debug(`Success: User was alerted to a status change in their check via SMS Msg ${msg}`);
        } else {
            debug(`Error: Could not send sms alert to the user who had a state change in their check, ${err}`);
        }
...
workers.asyncLog = (originalCheckData, checkOutcome, state, alertWarranted, timeofCheck) => {
    ...
    _logs.asyncAppend(logFileName, logString)
        .then(result => debug('Logging Result:', result))
        .catch(error => debug('Logging Error:', error));
        ...
        .then(logs => logs.map(log => {
            ...
            _logs.compress(logId, newLogId)
                .then(_logs.truncate(logId)
                        ...
                        .catch(error => debug('Error truncating a log', error)))
                .catch(error => debug('Error compressing one of the log files:', error))
        }))
        .catch(error => debug('Error rotating logs', error));
};
